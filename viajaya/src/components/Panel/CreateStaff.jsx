...
const CreateStaff = () => {
  ...
  const roleOptions = [
    {
      value: 1,
      label: "Cliente",
      description: "Cliente con cotizaciones",
      color: "bg-gray-100 text-gray-800",
    },
    {
      value: 2,
      label: "Asesor",
      description: "Vendedor básico",
  ...
  useEffect(() => {
    dispatch(fetchAllUsers()).then((result) => {
      if (result.payload) {
        ...
        const lideres = result.payload.filter(
          (user) => user.role === 3 && user.is_active
        );
        setAvailableLideres(lideres);

        // Filtrar gerentes (rol 4)
        const gerentes = result.payload.filter(
          (user) => user.role === 4 && user.is_active
        );
  ...
  const handleInputChange = (e) => {
    ...
    // ✅ CORREGIR: Convertir strings vacíos a null para evitar claves foráneas inválidas
    ...
    // ✅ MEJORADA: Lógica para manejar jerarquía automática
    if (name === "role") {
      const roleValue = parseInt(value);

      // ✅ Validar que el rol sea válido
      if (isNaN(roleValue) || roleValue < 1 || roleValue > 7) {
        console.warn("Rol inválido:", value);
        return;
      }
      ...
      // ✅ CORREGIR: Limpiar jerarquía según el rol con validación mejorada
      switch (roleValue) {
        case 1: // Cliente
        case 5: // Admin
        case 6: // Contador
        case 7: // Owner
          ...
          updates.is_active_seller = roleValue === 1; // Solo clientes no son vendedores
          break;

        case 2: // Asesor - requiere líder
          updates.lider_id = null; // Se limpia para selección manual
          updates.gerente_id = null; // Se asignará automáticamente al seleccionar líder
          updates.is_active_seller = true;
          break;

        case 3: // Líder - requiere gerente
          updates.lider_id = null; // Los líderes no tienen líder
          updates.gerente_id = null; // Se limpia para selección manual
          updates.is_active_seller = true;
          break;
        ...
        default:
          console.warn("Rol no contemplado:", roleValue);
          break;
      }

      console.log("🔄 Actualizando rol:", { roleValue, updates });
      setFormData((prev) => ({ ...prev, ...updates }));
    }

    // ✅ MEJORADO: Auto-asignar gerente cuando se selecciona líder
    if (name === 'lider_id' && value && value !== '') {
  const liderId = parseInt(value);
  
  // ✅ Validar que el ID sea válido
  if (isNaN(liderId)) {
    console.warn('ID de líder inválido:', value);
    return;
  }
  
  const selectedLider = availableLideres.find(lider => lider.id === liderId);
  
  if (selectedLider) {
    console.log('✅ Líder seleccionado:', selectedLider);
    
    // ✅ CORREGIR: Asegurar que los IDs sean enteros
    setFormData(prev => ({
      ...prev,
      lider_id: liderId, // Ya es entero
      gerente_id: selectedLider.gerente_id || null // Asegurar que sea entero o null
    }));
    
    console.log('✅ Gerente auto-asignado:', selectedLider.gerente_id);
  } else {
    console.error('❌ Líder no encontrado en la lista disponible');
    setFormData(prev => ({
      ...prev,
      lider_id: null,
      gerente_id: null
    ...
    // ✅ NUEVO: Validar gerente seleccionado manualmente
    if (name === "gerente_id" && value && value !== "") {
      const gerenteId = parseInt(value);

      if (isNaN(gerenteId)) {
        console.warn("ID de gerente inválido:", value);
        return;
      }

      const selectedGerente = availableGerentes.find(
        (gerente) => gerente.id === gerenteId
      );

      if (selectedGerente) {
        console.log("✅ Gerente seleccionado:", selectedGerente);
        setFormData((prev) => ({
          ...prev,
          gerente_id: gerenteId,
        }));
      } else {
        console.error("❌ Gerente no encontrado en la lista disponible");
        // Limpiar la selección si el gerente no existe
        setFormData((prev) => ({
          ...prev,
          gerente_id: null,
        }));
    ...
    // ✅ NUEVO: Limpiar campos dependientes cuando se deselecciona
    if (name === "lider_id" && (!value || value === "")) {
      setFormData((prev) => ({
        ...prev,
        lider_id: null,
        gerente_id: null, // Limpiar gerente auto-asignado
      }));
      console.log("🧹 Limpiado líder y gerente");
    }
  ...
  const validateForm = () => {
    ...
    if (formData.numero_cuenta && !formData.banco) {
      newErrors.banco =
        "El banco es requerido si proporcionas número de cuenta";
    }
    if (formData.banco && !formData.numero_cuenta) {
      newErrors.numero_cuenta =
        "El número de cuenta es requerido si proporcionas banco";
    }

    // Validación de permisos
    if (currentUser?.role && parseInt(formData.role) >= currentUser.role) {
      newErrors.role =
        "No puedes crear usuarios con rol igual o superior al tuyo";
    }
...
const handleSubmit = async (e) => {
  ...
  setSaveLoading(true);
  try {
    // ✅ CORREGIR: Convertir IDs a números enteros
    const userData = {
      // Información básica
      ...
      // ✅ CORREGIR: Jerarquía organizacional - Convertir a enteros
      lider_id: formData.lider_id ? parseInt(formData.lider_id) : null,
    ...
    console.log('🔍 Debug datos procesados:', {
      ...
      lider_id_type: typeof userData.lider_id,
      gerente_id: userData.gerente_id,
      gerente_id_type: typeof userData.gerente_id,
      role: userData.role,
      role_type: typeof userData.role
    });

    // ✅ Validar que los IDs sean válidos
    if (userData.lider_id && isNaN(userData.lider_id)) {
      throw new Error('ID de líder inválido');
    }
    if (userData.gerente_id && isNaN(userData.gerente_id)) {
      throw new Error('ID de gerente inválido');
    }
  ...
  const selectedRole = roleOptions.find(
    (role) => role.value === parseInt(formData.role)
  );
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-gray-600">
                  Registra un nuevo miembro del equipo con estructura
                ...
                <div>
                  ...
                  >
                    ...
                    {roleOptions
                      .filter(
                        (role) =>
                          !currentUser?.role || role.value < currentUser.role
                      )
                      .map((role) => (
                  ...
                  {selectedRole && (
                    <div
                      className={`mt-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${selectedRole.color}`}
                    >
                ...
                {parseInt(formData.role) === 2 && formData.gerente_id && (
                  <div>
                    ...
                    <input
                      type="text"
                      value={(() => {
                        const gerente = availableGerentes.find(
                          (g) => g.id === formData.gerente_id
                        );
                        return gerente
                          ? `${gerente.name} ${gerente.lastname}`
                          : "";
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    UUID único que otros usuarios pueden usar para registrarse
                    bajo este usuario
                  </p>
                </div>
