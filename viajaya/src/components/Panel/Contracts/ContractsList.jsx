...
const ContractsList = () => {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 'completed': return faFileContract;
  ...
  const handleAction = async (action, contractId) => {
    switch (action) {
      ...
      case 'send':
        ...
        console.log('Enviar contrato:', contractId);
        break;
      case 'download':
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex items-center justify-between">
            ...
            <FontAwesomeIcon icon={faFileContract} className="text-blue-500 text-2xl" />
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex items-center justify-between">
            ...
            <FontAwesomeIcon icon={faDollarSign} className="text-blue-500 text-2xl" />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        {contracts.length === 0 ? (
          <div className="p-8 text-center">
            <FontAwesomeIcon icon={faFileContract} className="text-gray-300 text-4xl mb-4" />
                  ...
                  <div className="grid grid-cols-12 gap-4 items-center">
                    ...
                    <div className="col-span-2">
                      <div className="flex items-center">
                        ...
                        <div>
                          <div className="font-medium text-gray-900">{contract.clienteName}</div>
                    ...
                    <div className="col-span-2">
                      <div className="flex items-center">
                        ...
                        <div>
                          <div className="font-medium text-gray-900">{contract.destino}</div>
                          <div className="text-sm text-gray-500">desde {contract.origen}</div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="col-span-1">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(contract.status)}`}>
                        <FontAwesomeIcon icon={getStatusIcon(contract.status)} className="mr-1" />
                    ...
                    <div className="col-span-1">
                      ...
                      <div className="text-sm text-gray-500">
                        {contract.forma_pago === 'cuotas' ? 'Cuotas' : 'Contado'}
                      </div>
                    </div>
                      ...
                      <div className="flex items-center text-sm">
                        ...
                        <div>
                          <div className="text-gray-900">
                            {new Date(contract.fecha_inicio_viaje).toLocaleDateString('es-ES')}
                          </div>
                          <div className="text-gray-500">
                            al {new Date(contract.fecha_fin_viaje).toLocaleDateString('es-ES')}
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="col-span-1">
                      <div className="flex items-center gap-1">
                        ...
                        {(contract.status === 'draft' || contract.status === 'active') && (
                          <button
                            onClick={() => handleAction('send', contract.id)}
                  ...
                  {contract.asesor && (
                    <div className="mt-2 pl-4 border-l-2 border-blue-200">
                      <div className="text-xs text-gray-600">
                        <span className="font-medium">Asesor:</span> {contract.asesor.name} {contract.asesor.lastname}
                        {contract.lider && (
                          <span className="ml-4">
                            <span className="font-medium">Líder:</span> {contract.lider.name} {contract.lider.lastname}
                          </span>
                        )}
                      </div>
                    </div>
