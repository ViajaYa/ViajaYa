...
const ContractPaymentDetail = ({ contractId, isOpen, onClose, onPaymentSuccess }) => {
  ...
  // ✅ Determinar si el contrato es de cuotas o contado - DEBE ir antes de los useEffect
  const isInstallmentPayment = currentContract?.forma_pago === 'cuotas';
  ...
  // ✅ Efecto para manejar cambio de pestaña cuando no es pago en cuotas
  useEffect(() => {
    if (currentContract && !isInstallmentPayment && activeTab === 'schedule') {
      setActiveTab('payments');
    }
  }, [currentContract, isInstallmentPayment, activeTab]);
          ...
          <div>
            ...
            <div className="font-medium">
              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                ...
                {isInstallmentPayment ? '📅 Pago en Cuotas' : '💳 Pago de Contado'}
              </span>
            </div>
          </div>
          <div>
    ...
    <div className="space-y-6">
      ...
      {!isInstallmentPayment && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          <div className="flex items-center">
            ...
            <div>
              <h4 className="text-sm font-medium text-green-800">Pago de Contado</h4>
              <p className="text-sm text-green-700">
                Este contrato requiere pago completo de ${formatCurrency(resumenFinanciero?.precio_total || 0)}
              </p>
            </div>
          </div>
        </div>
      ...
      {isInstallmentPayment && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="flex items-center">
            ...
            <div>
              <h4 className="text-sm font-medium text-blue-800">Pago en Cuotas</h4>
              <p className="text-sm text-blue-700">
                Puedes registrar pagos de cuotas individuales o abonos parciales. 
                Ve al &quot;Plan de Pagos&quot; para seleccionar una cuota específica.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">
          {isInstallmentPayment ? 'Registrar Pago de Cuota' : 'Registrar Pago de Contado'}
        </h3>
        <form onSubmit={handleNewPayment} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                ...
                {!isInstallmentPayment && (
                  <button
                    type="button"
                    onClick={() => setNewPayment({...newPayment, amount: (resumenFinanciero?.saldo_pendiente || 0).toString()})}
                    className="ml-2 text-xs text-blue-600 hover:text-blue-800 underline"
                  >
                    (Usar saldo pendiente: ${formatCurrency(resumenFinanciero?.saldo_pendiente || 0)})
                  </button>
                )}
              </label>
              <input
                ...
                placeholder={isInstallmentPayment ? "Monto de la cuota" : "Monto total del contrato"}
                required
              />
              {!isInstallmentPayment && newPayment.amount && (
                <p className="text-xs text-gray-500 mt-1">
                  {parseFloat(newPayment.amount) === parseFloat(resumenFinanciero?.saldo_pendiente || 0) 
                    ? "✅ Pago completo del contrato" 
                    : `Saldo restante: ${formatCurrency((resumenFinanciero?.saldo_pendiente || 0) - parseFloat(newPayment.amount || 0))}`
                  }
                </p>
              )}
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500"
                placeholder={isInstallmentPayment ? "Ej: Pago de cuota 1, Abono parcial..." : "Ej: Pago completo del contrato"}
              />
            </div>
          ...
          >
            {isInstallmentPayment ? '📅 Registrar Pago de Cuota' : '💳 Registrar Pago Completo'}
          </button>
        </form>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-6xl w-full max-h-[90vh] overflow-y-auto m-4">
        ...
        <div className="border-b border-gray-200">
          <nav className="flex">
            ...
            {/* Solo mostrar Plan de Pagos si es pago en cuotas */}
            {isInstallmentPayment && (
              <button
            ...
            >
              💳 {isInstallmentPayment ? 'Registrar Pagos' : 'Pago de Contado'}
            </button>
          </nav>
        </div>
        ...
        <div className="p-6">
          ...
          {activeTab === 'schedule' && isInstallmentPayment && renderScheduleTab()}
